    ...
    [ pageHeader "Players" "Team roster"
      if List.isEmpty players then
       ...
       div [ _class "overflow-x-auto" ] [
        table [ _class "w-full min-w-[24rem] text-left text-sm" ] [
    ...
    section [ _class "mt-8" ] [
        ...
        div [ _class "overflow-x-auto rounded-lg bg-card ring-1 ring-card-ring" ] [
            table [ _class "w-full text-center text-sm" ] [
                ...
                tbody [] [
                    tr [] [ for _, value in cells -> td [ _class "whitespace-nowrap px-3 py-3 font-semibold text-ink-strong" ] [ str value ] ]
                ]
            ]
        ]
    ]
...
let private battingTable (s: BattingStats) =
    statTable "Batting" [
        "G", string s.Games; "PA", s.PA; "AB", s.AB; "R", s.R; "H", s.H
        "2B", s.Doubles; "3B", s.Triples; "HR", s.HR; "RBI", s.RBI; "TB", s.TB
...
let private fieldingTable (s: FieldingStats) =
    statTable "Fielding" [
        "G", string s.Games; "IP", s.Innings; "PO", s.Putouts; "A", s.Assists
        "OA", s.OutfieldAssists; "E", s.Errors; "DP", s.DoublePlays; "PB", s.PassedBalls
        "SB Att", s.StealAttempts; "CS", s.CaughtStealing; "RF", s.RangeFactor; "FPCT", s.FieldingPct
    ]

let private pitchingTable (s: PitchingStats) =
    statTable "Pitching" [
        "G", s.Games; "GS", s.Starts; "IP", s.IP; "H", s.H; "R", s.R; "ER", s.ER
        "BB", s.BB; "SO", s.SO; "HBP", s.HBP; "WP", s.WildPitches; "W–L", s.Record
        "SV", s.Saves; "BF", s.BattersFaced; "OPP AVG", s.OppAVG; "WHIP", s.WHIP; "ERA", s.ERA
    ]
...
let detailView (pl: Player) (stats: PlayerStats) : XmlNode list =
    [ a [ _href "/players"; _class "text-sm font-semibold text-accent-text hover:underline" ] [ str "← All players" ]
      ...
      div [ _class "grid grid-cols-2 gap-4 sm:grid-cols-3" ] [
      ...
      | Some pi -> pitchingTable pi
      | None -> ()
      if stats.Batting.IsNone && stats.Fielding.IsNone && stats.Pitching.IsNone then
          p [ _class "mt-8 rounded-lg bg-card p-6 text-ink-muted ring-1 ring-card-ring" ] [
              str "No season stats yet."
          ] ]
